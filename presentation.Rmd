...
`geom_col()` creates a bar chart where the heights of the bars represents the values of the data, or `stat_identity()`.
...
Position refers to ways adjust the location or orientation of your data.
...
Split your plot up into subplots, or facets,  that each display one subset of the data. This is particularly useful for categorical variables.
...
`ggpairs()` is a function from the `GGally` package that 

* provides two different comparisons of each pair of columns, and 
